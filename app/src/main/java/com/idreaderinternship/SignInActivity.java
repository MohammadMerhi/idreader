...
public class SignInActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        MY_KEY = (shared.getInt("MY_KEY", 0));
        if (MY_KEY == 1) {
            Intent goToPinCode = new Intent(this, PINCodeActivity.class);
            startActivity(goToPinCode);
            finish();
        } else if (MY_KEY == 0) {
            signOut();
        } else {
...
//                                    finishAffinity();
                                    ...
                                    int guardID = jsonObject.getInt("guardId");

                                    SharedPreferences shared = getSharedPreferences(PREF_NAME, MODE_PRIVATE);
                                    String pin = (shared.getString("PIN CODE", ""));

                                    SharedPreferences.Editor editor = shared.edit();
                                    editor.putInt("Guard ID", guardID);
                                    editor.putInt("MY_KEY", 1);
                                    editor.apply();
